...
`Expose` 어트리뷰트를 필드, 프로퍼티, 주 생성자(primary constructor) 매개변수에 적용하면 어트리뷰트에 지정한 인터페이스를 구현하는 코드를 생성합니다. 어트리뷰트를 사용하는 타입은 `partial`로 선언되어야 합니다.
...
메서드, 프로퍼티, 인덱서, 이벤트를 위임 대상으로 지원합니다.
...
### `Lift` 어트리뷰트를 사용하여 필드 혹은 프로퍼티의 멤버를 선언된 타입에 위임하기
